...
function TriviaBot.ShowHint()
    -- Only if we want hints:
    local qid = TB_Question_List[ TB_Question_Order[ TB_Active_Question ] ];
    if TriviaBot_Config['Show_Hints'] and TB_Questions['Hints'][qid] then
        -- For simplicity, just show the first hint or increment a hint counter
        local firstHint = TB_Questions['Hints'][qid][1];
        if firstHint then
            TriviaBot.Send("Hint: " .. firstHint);
        end
    end
end

function TriviaBot.TimeUp()
    -- Stop accepting answers
    TB_Accept_Answers = false;

    -- Optionally reveal the correct answer or just say "Time's up!"
    local qid = TB_Question_List[ TB_Question_Order[ TB_Active_Question ] ];
    local answerList = TB_Questions['Answers'][qid];
    if answerList and #answerList > 0 then
        TriviaBot.Send("Time's up! The correct answer was: " .. answerList[1]);
    else
        TriviaBot.Send("Time's up! (No known answer?)");
    end
end
...
----------------------------------------------------------------------------
-- Ask a question (Manual Version)
----------------------------------------------------------------------------
function TriviaBot.AskQuestion(announce)
    -- Advance to the next question index
    TB_Active_Question = TB_Active_Question + 1

    -- If we've exceeded the available questions, reshuffle
    if (TB_Active_Question > #TB_Questions['Question']) then
        TriviaBot.Randomize()
        TB_Active_Question = 1
        if not TB_limit_out then
            TriviaBot.Send(L.TB_SEND_OUTOFQUESTIONS)  -- "Out of questions... Reshuffling"
        end
        TriviaBot.Print(L.TB_PRINT_OUTOFQUESTIONS)
    end

    -- If round size is finite, track which question # in this round
    local questionNumber = 0
    if (TriviaBot_Config['Round_Size'] ~= TB_Infinite_Round) then
        TB_Round_Counter = (TB_Round_Counter or 0) + 1
        questionNumber   = TB_Round_Counter
    end

    -- Identify the question ID from your random ordering
    local setid = TriviaBot_Config['Question_Set']
    local qid   = TB_Question_List[ TB_Question_Order[TB_Active_Question] ]

    -- Optionally announce category info
    if (announce and not TB_limit_out) then
		local categoryId = TB_Questions['Category'][qid]  -- e.g. "1"
		local numericCat = tonumber(categoryId)           -- e.g. 1
		local catName    = TB_Question_Sets[setid]['Categories'][numericCat]
		if not catName then
			catName = "Unknown Category"
		end
		local setTitle = TriviaBot.Capitalize(TB_Question_Sets[setid]['Title'])
		TriviaBot.Send("C: " .. setTitle .. " - " .. catName)
    end

    -- Broadcast the question text
    local questionText = TB_Questions['Question'][qid]
	TB_Question_Starttime = GetTime();
    if (questionNumber > 0) then
        TriviaBot.Send("Q" .. questionNumber .. ": " .. questionText)
    else
        TriviaBot.Send("Q: " .. questionText)
    end

    -- Let the addon accept answers again
    TB_Accept_Answers = true

    -- If you have a Skip button, enable it
...
function TriviaBot.TimeUpOrAnnounce()
    if TB_Winner then
        -- Announce the winner AND the time
        local announcement = TB_Winner .. " got it right!"
        
        -- If we have a stored time, add it in
        if TB_WinnerTime then
            announcement = announcement .. " (" .. TB_WinnerTime .. "s)"
        end
        
        TriviaBot.Send(announcement)

        -- Then end the question
        TriviaBot.EndQuestion(false)

        -- Reset TB_Winner + TB_WinnerTime
        TB_Winner = nil
        TB_WinnerTime = nil
    else
        -- No winner => "Time's Up" logic
        TriviaBot.TimeUp()
    end

    -- Refresh the button text for the next question
    TriviaBot.UpdateTimeUpButton()
end
...
function TriviaBot.CheckAnswer(player, msg)
    -- 1) If we already have a winner for this question, ignore all subsequent correct answers
    if TB_Winner then
        return
    end

    -- 2) Don't count answers that come from the add-on's own messages
    if string.find(msg, TB_Channel_Prefix, 1, true) then
        return
    end

    -- 3) Clean up the player's message (remove special chars, links, punctuation)
    msg = TriviaBot.StringCorrection(msg)

    -- 4) Identify the question being asked
    local qid = TB_Question_List[ TB_Question_Order[ TB_Active_Question ] ]

    -- 5) Check each possible correct answer
    for i = 1, #TB_Questions['Answers'][qid] do
        local correctAnswer = TB_Questions['Answers'][qid][i]

        -- If message contains the correct answer substring
        if string.find(string.lower(msg), string.lower(correctAnswer), 1, true) then
            -- Mark this player as the winner
            TB_Winner = player

			-- Update the button text from "Time’s Up" to "Announce Winner" if TB_Winner is set
			TriviaBot.UpdateTimeUpButton()

            -- Cancel any scheduled timeouts / warnings
            TriviaBot.UnSchedule("all")

            ------------------------------------------------
            -- OLD SCOREKEEPING + TIMING LOGIC (unchanged)
            ------------------------------------------------
            local timeTaken = GetTime() - TB_Question_Starttime
            timeTaken = math.floor(timeTaken * 100 + 0.5) / 100

			TB_WinnerTime = timeTaken;

            -- If you like, you can still do a LOCAL print:
            -- print("'".. msg .."' is correct! " .. player .. " answered in " .. timeTaken .. " seconds.")
            -- but comment out or remove the direct public send:
            ...
            -- Initialize personal score tables if missing
            ...
            if (TB_Game_Scores['Temp_Win_Streak']['Holder'] == player) then
                ...
                -- Start new streak for this player
                TB_Game_Scores['Temp_Win_Streak']['Holder'] = player
                TB_Game_Scores['Temp_Win_Streak']['Count']  = 1
            end

            -- New game win streak record
            if (not TB_Game_Scores['Best_Win_Streak']['Holder']
                or TB_Game_Scores['Temp_Win_Streak']['Count'] > TB_Game_Scores['Best_Win_Streak']['Count']) then

                TB_Game_Scores['Best_Win_Streak']['Holder'] = player
                TB_Game_Scores['Best_Win_Streak']['Count']  = TB_Game_Scores['Temp_Win_Streak']['Count']

                if (TriviaBot_Config['Report_Win_Streak']
                    and TB_Game_Scores['Best_Win_Streak']['Count'] % 5 == 0
                    and not TB_limit_out) then
            ...
            if (not TriviaBot_Scores['Win_Streak']['Holder']
                or TB_Game_Scores['Temp_Win_Streak']['Count'] > TriviaBot_Scores['Win_Streak']['Count']) then
            ...
            --------------------------------------------------------
            -- 6) DO NOT end the question automatically
            -- Instead, we rely on a button to do "Announce Winner"
            -- or "Time's Up" manually.
            --------------------------------------------------------
            -- TriviaBot.EndQuestion(false)   <-- remove or comment out
            -- Instead, you can do:
            -- TriviaBot.UpdateTimeUpButton() if you have that function
            -- to switch the button label from "Time's Up" to "Announce Winner".
            if TriviaBot.UpdateTimeUpButton then
                TriviaBot.UpdateTimeUpButton()
            end

            -- We found a correct answer, so break out of the loop
            break
        end
    end
end

function TriviaBot.UpdateTimeUpButton()
    if TB_Winner then
        TriviaBotGUI.TimeUpButton:SetText("Announce Winner")
    else
        TriviaBotGUI.TimeUpButton:SetText("Time's Up")
    end
end
...
----------------------------------------------------------------------------
-- End the current question (Manual Version)
----------------------------------------------------------------------------
...
print("TriviaBot.InitEnd was called!");
	-- Initialize the GUI
...
----------------------------------------------------------------------------
-- Skip current question (Manual Version)
----------------------------------------------------------------------------
function TriviaBot.SkipQuestion()
    if (not TB_Running) then
        TriviaBot.PrintError(L.TB_ERROR_NOGAME);
        return;
    end

    -- Disable skip so we don't press it twice
    if (TriviaBotGUI.SkipButton) then
        TriviaBotGUI.SkipButton:Disable()
    end

    if not TB_limit_out then
        TriviaBot.Send(L.TB_SEND_QUESTIONSKIPPED)  -- "Question Skipped!"
    end

    -- Mark that we aren't taking more answers for this question
    TB_Accept_Answers = false

    -- Optionally show the correct answer, if configured
    if (TriviaBot_Config['Show_Answers']) then
        TriviaBot.PrintAnswers()
    end

    TriviaBot.Print(L.TB_PRINT_QUESTIONSKIP)  -- local console msg "Question Skipped!"

    -- NO scheduling for the next question!
    -- The user must manually click the "Next Question" button to ask another.
end

----------------------------------------------------------------------------
-- Start trivia session (Manual Version)
----------------------------------------------------------------------------
function TriviaBot.Start(announce)
    local id = TriviaBot_Config['Question_Set'];

    -- If this is a 'stub' set (LOD question pack not loaded), bail out
    if TriviaBot_Questions[id]['Stub'] and not TriviaBot.questionmaker then
        TriviaBot.PrintError(L.TB_ERROR_NOLOADED)
        return
    end

    -- Mark trivia as running
    TB_Running = true;

    -- Ensure we can broadcast in the chosen chat
    TriviaBot.CheckChannel();

    -- Determine whether to limit output for “General” (to avoid spam)
    TB_limit_out = strlower(TriviaBot_Config['Chat_Type']) == "general"

    -- Announce the start (all at once, in the same hardware-event call)
    if not TB_limit_out then
        TriviaBot.Send(L.TB_SEND_POWEREDBY);  -- "Powered by TriviaBot..."
    end

    -- Figure out the category name (or ALL if 0)
    local category;
    ...
    -- Announce database info
    if not TB_limit_out then
        TriviaBot.Send(L.TB_SEND_USINGDATABASE .. TB_Question_Sets[id]['Title']);
    end
    if (announce and not TB_limit_out) then
    ...
    -- If round size is limited, announce that
    if (TriviaBot_Config['Round_Size'] ~= TB_Infinite_Round) then
        if not TB_limit_out then
            TriviaBot.Send(L.TB_SEND_STARTROUND 
                .. TriviaBot_Config['Round_Size'] 
                .. L.TB_SEND_BLANKQUESTIONS);
        end
    end

    -- Print a note to your own chat that the first question is ready
    if (announce) then
        TriviaBot.Print(L.TB_PRINT_FIRSTQUESTION);
    end

    -- Clear game scores for a fresh session
    TB_Game_Scores = {
        Best_Win_Streak = {},
        Temp_Win_Streak = {},
        Speed           = {},
    ...
    -- If all-time scores table is missing sections, create them
    ...
    -- Update GUI (so the button text changes to "Stop Trivia", etc.)
...
function TriviaBot.Stop(announce)
    ...
    -- Announce that we’re ending
    TriviaBot.Send("The Game has Concluded!")
	TriviaBot.Report("endreport")
    TriviaBot.Send("Thanks for playing!")
...
function TriviaBot.GUIInitialize()
	...
	TriviaBotGUI:SetBackdrop(
	{
		bgFile = "Interface/Tooltips/UI-Tooltip-Background",
		edgeFile = "Interface/Tooltips/UI-Tooltip-Border",
		tile = true, tileSize = 48, edgeSize = 16,
		insets = {left = 5, right = 5, top = 6, bottom = 6}
	});
	...
	TriviaBotGUI.QuestionList.PrintButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate");
	...
	TriviaBotGUI.CategoryList.PrintButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate");
	...
	TriviaBotGUI.ChannelButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate");
	...
	TriviaBotGUI.Channel:SetHeight(70);
	...
	TriviaBotGUI.ShowAnswersCheckBox = CreateFrame("CheckButton", nil, TriviaBotGUI, "UICheckButtonTemplate");
	...
	TriviaBotGUI.ShowReportsCheckBox = CreateFrame("CheckButton", nil, TriviaBotGUI, "UICheckButtonTemplate");
	...
	-- Show Hints Checkbox
	TriviaBotGUI.ShowHintsCheckBox = CreateFrame("CheckButton", nil, TriviaBotGUI, "UICheckButtonTemplate");
	...
	TriviaBotGUI.ShowWhispersCheckBox = CreateFrame("CheckButton", nil, TriviaBotGUI, "UICheckButtonTemplate");
	...
	TriviaBotGUI.ReportWinStreakCheckBox = CreateFrame("CheckButton", nil, TriviaBotGUI, "UICheckButtonTemplate");
	...
	TriviaBotGUI.ReportPersonalCheckBox = CreateFrame("CheckButton", nil, TriviaBotGUI, "UICheckButtonTemplate");
	...
	-- Point Mode Checkbox
	TriviaBotGUI.PointModeCheckBox = CreateFrame("CheckButton", nil, TriviaBotGUI, "UICheckButtonTemplate");
	...
	TriviaBotGUI.ShortChannelTagCheckBox = CreateFrame("CheckButton", nil, TriviaBotGUI, "UICheckButtonTemplate");
	...
	-- Start/Stop Button
	TriviaBotGUI.StartStopButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate");
	...
	-- Skip Button
	TriviaBotGUI.SkipButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate");
	...
	-------------------------------------------------------------------------------
	-- "Next Question" Button (Column 1, Top)
	-------------------------------------------------------------------------------
	TriviaBotGUI.NextQuestionButton = CreateFrame("Button", "TriviaBotGUI_NextQuestionButton", TriviaBotGUI, "UIPanelButtonTemplate")
	TriviaBotGUI.NextQuestionButton:SetSize(120, 22)
	TriviaBotGUI.NextQuestionButton:SetText("Next Question")
	-- This anchor is unchanged, placing it below Skip
	TriviaBotGUI.NextQuestionButton:SetPoint("TOPRIGHT", TriviaBotGUI.SkipButton, "BOTTOMRIGHT", 0, -5)
	TriviaBotGUI.NextQuestionButton:SetScript("OnClick", function()
		TriviaBot.AskQuestion(true)
	end)

	-------------------------------------------------------------------------------
	-- "Hint" Button (Column 1, Below Next Question)
	-------------------------------------------------------------------------------
	TriviaBotGUI.HintButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate")
	TriviaBotGUI.HintButton:SetSize(120, 22)  -- match Next Question
	TriviaBotGUI.HintButton:SetText("Hint")
	-- Place it 5 px below NextQuestion
	TriviaBotGUI.HintButton:SetPoint("TOPRIGHT", TriviaBotGUI.NextQuestionButton, "BOTTOMRIGHT", 0, -5)
	TriviaBotGUI.HintButton:SetScript("OnClick", function()
		TriviaBot.ShowHint()
	end)

	-------------------------------------------------------------------------------
	-- "Time Alert" Button (Column 2, Aligned with Next Question horizontally)
	-------------------------------------------------------------------------------
	TriviaBotGUI.TimeAlertButton = CreateFrame("Button", "TriviaBotGUI_TimeAlertButton", TriviaBotGUI, "UIPanelButtonTemplate")
	TriviaBotGUI.TimeAlertButton:SetSize(120, 22)
	TriviaBotGUI.TimeAlertButton:SetText("Time Alert!")
	-- Anchor it to the right of NextQuestion (same vertical alignment, 10 px gap)
	TriviaBotGUI.TimeAlertButton:SetPoint("TOPLEFT", TriviaBotGUI.NextQuestionButton, "TOPRIGHT", 10, 0)
	TriviaBotGUI.TimeAlertButton:SetScript("OnClick", function()
		TriviaBot.Send("Time is running out! 10 seconds left!")
	end)

	-------------------------------------------------------------------------------
	-- "Time's Up" Button (Column 2, Below Time Alert)
	-------------------------------------------------------------------------------
	TriviaBotGUI.TimeUpButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate")
	TriviaBotGUI.TimeUpButton:SetSize(120, 22)
	TriviaBotGUI.TimeUpButton:SetText("Time's Up")
	-- Place it 5 px below TimeAlert
	TriviaBotGUI.TimeUpButton:SetPoint("TOPLEFT", TriviaBotGUI.TimeAlertButton, "BOTTOMLEFT", 0, -5)
	TriviaBotGUI.TimeUpButton:SetScript("OnClick", function()
		TriviaBot.TimeUpOrAnnounce()
	end)


	-- Game Score Button
	TriviaBotGUI.GameScoresButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate");
	...
	if IsAddOnLoaded("TriviaBot_QuestionMaker") then
		TriviaBotGUI.QuestionMakerButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate");
		...
		TriviaBotGUI.ReloadQuestionsButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate");
	...
	TriviaBotGUI.AllTimeScoresButton = CreateFrame("Button", nil, TriviaBotGUI, "UIPanelButtonTemplate");
